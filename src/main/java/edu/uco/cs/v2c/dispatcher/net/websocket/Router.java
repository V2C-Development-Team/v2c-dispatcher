...
public class Router implements RoutingListener {
  ...
  @Override public void onRoute(Session module, String message, String moduleName) {
    if(module == null) V2CDispatcher.getLogger().logError(LABEL,
      ...
      try {
        WebSocketHandler.dispatch(module,
            new RouteCommandPayload().setRecipient(moduleName).setCommand(message).serialize());
      } catch(MalformedPayloadException e) {}
      V2CDispatcher.getLogger().logDebug(LABEL,
          String.format("Sent message (%1$s), to a registered session at %2$s:%3$d", message,
              module.getRemoteAddress().getHostString(), module.getRemoteAddress().getPort()));
    }
  }
